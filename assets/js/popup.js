document.addEventListener("DOMContentLoaded", function () {
  ...
  function showStep(step) {
    ...
    const currentContent = document.querySelector(
      `.csp-step-content[data-step="${step}"]`
    );
    if (currentContent) {
  ...
  function openModal() {
    ...
    if (!parseInt(window.csp_ajax_obj.is_logged_in)) {
  ...
  function validateStep(step) {
    ...
    let isValid = true;
    const requiredInputs = currentContent.querySelectorAll(
  ...
  if (submitBtn) {
    submitBtn.addEventListener("click", function (e) {
      ...
      if (!parseInt(window.csp_ajax_obj.is_logged_in)) {
        localStorage.setItem("csp_measurements", JSON.stringify(data));
        alert(
          "Vos mensurations ont été enregistrées localement. Connectez-vous pour les sauvegarder définitivement."
        );
        closeModal();
        return;
      }
      ...
      fetch(window.csp_ajax_obj.ajax_url, {
        ...
        .then((res) => {
          if (res.success) {
            ...
            alert(
              "Erreur : " + (res.data?.message || "Une erreur est survenue")
            );
          }
        })
        .catch((err) => {
